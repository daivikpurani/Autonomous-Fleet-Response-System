#!/usr/bin/env python3
"""Launcher script for operator-service.
...
class ServiceModuleLoader(importlib.machinery.SourceFileLoader):
    ...
    def create_module(self, spec):
        ...
        if module is not None and spec.name.startswith("operator_service."):
...
# Create a custom import hook for operator_service package
class ServiceImportFinder:
    """Custom finder that maps operator_service.* to files in service_dir."""
    
    @staticmethod
    def find_spec(name, path=None, target=None):
        if name.startswith("operator_service."):
            parts = name.split(".")
            if len(parts) == 2:
                # e.g., operator_service.config
                module_name = parts[1]
                ...
                # e.g., operator_service.db.models
...
import types
fake_pkg = types.ModuleType("operator_service")
fake_pkg.__path__ = [str(service_dir)]
sys.modules["operator_service"] = fake_pkg

# Load main.py as a module
main_file = service_dir / "main.py"
spec = importlib.util.spec_from_file_location("operator_service.main", main_file)

module = importlib.util.module_from_spec(spec)
module.__package__ = "operator_service"
module.__name__ = "operator_service.main"
module.__file__ = str(main_file)
sys.modules["operator_service.main"] = module

# Execute the module - this will process all the imports
spec.loader.exec_module(module)
app = module.app

if __name__ == "__main__":
    import uvicorn
    port = int(sys.argv[1]) if len(sys.argv) > 1 and sys.argv[1].isdigit() else 8003
